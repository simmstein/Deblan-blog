...
class sfValidatedAvatarFile extends sfValidatedFile {
    ...
    if (!is_writable($directory))
    {
      ...
      throw new Exception(sprintf('File upload path "%s" is not writable.', $directory));
		...
		$img = new sfImage($this->getTempName(), image_type_to_mime_type(exif_imagetype($this->getTempName()))); 
		$img->resize(250, $img->getHeight()*250/$img->getWidth());
		$img->save();
