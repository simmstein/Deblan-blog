...
function include_compressed_javascripts()
{
	require_once sfConfig::get('sf_root_dir').DIRECTORY_SEPARATOR.'lib'.DIRECTORY_SEPARATOR.'jsmin.class.php';

  $response = sfContext::getInstance()->getResponse();
  sfConfig::set('symfony.asset.javascripts_included', true);

	$web_js_root = sfConfig::get('sf_root_dir').DIRECTORY_SEPARATOR.'web'.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR;
	$filename = $web_js_root.'compressed_javascript.js';
	...
	if(!preg_match('`local.`', $_SERVER['SERVER_NAME'])) {
		if(!file_exists($filename)) {
			...
			echo javascript_include_tag('compressed_javascript.js');
		}
	...
	foreach($response->getJavascripts() as $file => $options) {
		$compressed_javascript.= file_get_contents($web_js_root.$file).";\n\n";
	}

	$newMd5js = md5($compressed_javascript);
	...
	if(file_exists($filename)) {
		...
		if($md5js == $newMd5js) {
			echo javascript_include_tag('compressed_javascript.js?'.$md5js);
			return true;
		}
	}

	$compressed_javascript = '// '.$newMd5js."\n".JSMin::minify($compressed_javascript);
	file_put_contents($filename, $compressed_javascript);
	echo javascript_include_tag('compressed_javascript.js?'.$newMd5js);
}
...
function include_compressed_stylesheets() 
{
  $response = sfContext::getInstance()->getResponse();
  sfConfig::set('symfony.asset.stylesheets_included', true);
	$web_css_root = sfConfig::get('sf_root_dir').DIRECTORY_SEPARATOR.'web'.DIRECTORY_SEPARATOR.'css'.DIRECTORY_SEPARATOR;
	$filename  = $web_css_root.'compressed_css.css';
	...
	if(!preg_match('`local.`', $_SERVER['SERVER_NAME'])) {
		if(!file_exists($filename)) {
			...
			echo stylesheet_tag('compressed_css.css');	
		}
  ...
  foreach ($response->getStylesheets() as $file => $options)
  {
    $md5s[] = md5_file($web_css_root.$file);
  }
	
	$newMd5css = serialize($md5s);
	$filename  = $web_css_root.'compressed_css.css';

	if(file_exists($filename)) {
		$lines = file($filename);
		$md5css = unserialize(trim($lines[0], '/ '));

		if($md5css == $newMd5css) {
			echo stylesheet_tag('compressed_css.css?'.$md5css);
			return true;
		}		
	}
	...
	foreach($response->getStylesheets() as $file => $options) {
		$compressed_css.= shell_exec('lessc '.$web_css_root.$file.' -x')."\n";
	}


	$compressed_css = '// '.$newMd5css."\n".$compressed_css;
	file_put_contents($filename, $compressed_css);
	echo stylesheet_tag('compressed_css.css?'.md5($newMd5css));	
}
