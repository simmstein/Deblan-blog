<?php if($comment): if(($comment->hasParent() && $level > 1) || !$comment->hasParent()):  ?>
	...
	<div class="comment" style="padding-left:<?php echo ($level-1)*20; ?>px">
		<article id="c<?php echo $comment->getId(); ?>">
			<header>
				<div class="comment_author">
					...
					<a rel="nofollow" href="#c<?php echo $comment->getId(); ?>">#</a>
				</div>
	...
	<?php if($comment->hasChildren()): ?>
		<?php foreach($comment->getChildren() as $commentC): ?>
			<?php include_component('page', 'showComment', array('level' => ($level+1), 'comment' => $commentC)); ?>
		<?php endforeach; ?>
	<?php endif; ?>
<?php endif; endif; ?>
